...
def laurent_sparse_tensor_from_path(
    path, vars=["t", "q"], compressed=False, sage_polynomials=False
):
    if compressed:
        import bz2

        with bz2.open(path, "rt") as f:
            return laurent_sparse_tensor_from_file(
                f, vars=vars, sage_polynomials=sage_polynomials
            )
    else:
...
class RMatrix:
    ...
    def from_directory(
        ...
        tensors = [
            laurent_sparse_tensor_from_path(
...
def colored_links_gould_R_matrices(n, sage_polynomials=False):
    if 0 < n <= 4:
        ...
        if key in _cache.keys():
            ...
            _cache[key] = RMatrix.from_directory(
                dir_path=os.path.join(dir_path, f"R_matrices/V{n}/"),
                vars=["t", "q"],
                compressed=False,
                sage_polynomials=sage_polynomials,
            )
...
def colored_jones_R_matrices(n, sage_polynomials=False):
    ...
    if not sage_polynomials:
        hp = SparseTensor(
            (n, n), data={(i, i): _q_pow(4 * i - 2 * (n - 1)) for i in range(n)}
        )
        hn = SparseTensor(
            (n, n), data={(i, i): _q_pow(2 * (n - 1) - 4 * i) for i in range(n)}
        )
    else:
        hp = SparseTensor(
            (n, n),
            data={(i, i): _q_pow(4 * i - 2 * (n - 1)).to_sage() for i in range(n)},
        )
        hn = SparseTensor(
            (n, n),
            data={(i, i): _q_pow(2 * (n - 1) - 4 * i).to_sage() for i in range(n)},
        )
